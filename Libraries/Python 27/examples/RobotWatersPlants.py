'''
This is an advanced example that performs the "Watering Plants" task 
for the Multipurpose Mobile Manipulator robot. 
...
def waterPlant(angle):
  ...
  if(angle > 90):
    mmm.setWheelVelocity(-.18,.18) 
    rotTime = rot90 * abs(90.0-angle)/300.0
    time.sleep(rotTime)
  elif(angle < 90):
    mmm.setWheelVelocity(-.18,.18)
...
for heading in xrange(headings-1):
